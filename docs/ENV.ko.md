...
`CPM_ANUBIS_IMAGE`는 전용 실행기가 포함된 Anubis 이미지 태그다. `ANUBIS_PRIVATE_KEY_HEX`는 계속 루트 `.env`에서 관리한다. 챌린지·쿠키 유지시간·언어 등의 비밀이 아닌 운영 옵션은 [CPM Settings](ANUBIS.ko.md)에서 관리한다.
